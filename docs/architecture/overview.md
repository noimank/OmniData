...
单 Browser + 多 Context 架构，内置 7×24 自愈机制。

- **单例模式**：全局唯一的浏览器实例（`@lru_cache` 单例）
- **按命名空间缓存**：每个数据源一个常驻 Context 复用，无容量限制
- **空闲回收**：Context 空闲 10 分钟自动回收
- **整体回收**：浏览器存活满 8 小时或累计建页 5000 次，在空闲窗口整体重启
- **自愈**：浏览器崩溃自动重启，launch 失败重建驱动
- **状态恢复**：登录态从 Redis 自动恢复，回收/重启对上层无感
