...
class ETFRankingSpider(BaseWebSpider):
    ...
    def _parse_etf(self, item: dict) -> dict:
        ...
        result = {
            ...
            "最新价": round(safe_float(item.get("f2")) / 100, 4),
            "涨跌幅": round(safe_float(item.get("f3")) / 100, 2),
            "涨跌额": round(safe_float(item.get("f4")) / 100, 4),
            "成交量(手)": safe_int(item.get("f5")),
            "成交额(万元)": round(safe_float(item.get("f6")) / 10000, 2),
            "最高": round(safe_float(item.get("f15")) / 100, 4),
            "最低": round(safe_float(item.get("f16")) / 100, 4),
            "今开": round(safe_float(item.get("f17")) / 100, 4),
            "昨收": round(safe_float(item.get("f18")) / 100, 4),
