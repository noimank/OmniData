    ...
    async def crawl(self, params: FundNAVParams) -> SpiderResult:
        ...
        try:
            async with self.new_page("eastmoney") as page:
                ...
                # ── 分页获取全量数据 ──
                # 使用 page.context.request 绕过浏览器 CORS 限制，同时自动携带 context cookies
                ...
                if params.data_format == "markdown":
                    ...
                    return SpiderResult(
                        success=True, data=df.to_markdown(index=False), message=message
                    )
                if params.data_format == "string":
                    df = pd.DataFrame(all_records)
                    return SpiderResult(
                        success=True, data=df.to_string(index=False), message=message
                    )
