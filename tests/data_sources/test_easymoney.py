...
class TestEastMoneySpider:
    ...
    async def test_run32(self, browser_pool):
        """测试板块当日异动爬虫 - 融资融券板块"""
    ...
    async def test_run33(self, browser_pool):
        """测试板块当日异动爬虫 - 融资融券板块"""
        spider_name = "eastmoney_global_index_ranking"
        params = {"page": 1, "page_size": 60, "sort_order": "desc", "region": "all"}
        register = spider_register()
        instance = register.get_spider_instance(spider_name)
        res = await instance.run(params)
        print(res)

    async def test_run34(self, browser_pool):
        """测试板块当日异动爬虫 - 融资融券板块"""
        spider_name = "eastmoney_index_intraday"
        params = {"index_code": "000001", "limit": 0}
        register = spider_register()
        instance = register.get_spider_instance(spider_name)
        res = await instance.run(params)
        print(res)

    async def test_run35(self, browser_pool):
        """测试板块当日异动爬虫 - 融资融券板块"""
    ...
    async def test_run37(self, browser_pool):
        """测试外汇市场行情爬虫"""
        spider_name = "eastmoney_stock_performance_report"
        params = {"page": 1, "page_size": 20, "stock_code": "688313"}
        register = spider_register()
        instance = register.get_spider_instance(spider_name)
        res = await instance.run(params)
        print(res)
